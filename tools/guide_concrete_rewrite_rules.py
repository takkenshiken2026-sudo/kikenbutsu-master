...
FIELD_GUIDE_FORBIDDEN_RE = re.compile(
    r"5行表|7行表|5列表|三列表|/terms/|Day3解き直し|Day0→3→7|"
    r"11/22新規0|9月通し\d+/50|9/\d+通し\d+/\d+"
)

FIELD_GUIDE_SUBSTANCE_RE = re.compile(
    r"条文|論点|制度|消防法|危険物|政令|指定数量|倍数|届出|許可|禁制|"
    r"引火点|閃点|蒸気圧|比重|混合|静電|自然発火|"
    r"消火|漏えい|貯蔵|タンク|禁水|禁泡|火災|"
    r"品名|設備基準|三領域|横断|分野またぎ"
)
...
def validate_field_guide_genre(slug: str, patch: dict[str, str]) -> list[str]:
    ...
    if FIELD_GUIDE_FORBIDDEN_RE.search(combined):
        errors.append(
            ...
            f"(三列表, /terms/, Day3, 9/6通し 等). Link to study-plan instead."
        )
